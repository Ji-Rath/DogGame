...
onewayending()
	...
	distancePlayer = follow.x - x;
	distancePlayerheight = follow.y - y;
	...
	/*
		...
		if (follow.y-y < -300) and (canjump != 0) and (topdetect == -4)
		{
	...
	else if (state == party1states.wait)
	{
		...
		#region
		if (distance > detectrange) and (follow.move != 0)
		{
	...
	else*/ if (!place_meeting(x,y+3,oWall)) // and (follow.move == 0)
	{
