...
if(type[page] == 1) and (portrait[page] != -1)
{
	//Variables we need
	...
	repeat(tpl)
	{
		...
		//Draw our choices
		image_speed = 0.5;
		draw_sprite_ext(speaker[page].face, image_index, pos_x-(portraitWidth/2), pos_y + (portraitHeight/2), sign(oPlayer.image_xscale)*scale,scale, 0, c_white, 1);
...
else if(portrait[page] != -1){
	
	image_speed = 0.5;
	
	draw_sprite_ext(speaker[page].face, image_index, pos_x-(portraitWidth/2), pos_y+(portraitHeight/2), (sign(speaker[page].image_xscale))*scale,scale, 0, c_white, 1);
...
#region TYPE 0: NORMAL DIALOGUE
	...
	if(charCount < str_len and !pause){
		...
		var txtspd = 0.35;
		if(text_speed_c+1 < text_speed_al and charCount == txtspd) {
			text_speed_c++;
			tsc2 = text_speed_c*2;
		}
		charCount += txtspd;
